...
{
    public class SixInRowState : IWinningState
    {
        ...
        private bool CheckRows(Proxy[,] pieces)
        {
            ...
            for (int row = 0; row < rows; row++)
            {
                ...
                {
                    if (CheckConsecutive(pieces[row, col], pieces[row, col + 1], pieces[row, col + 2], pieces[row, col + 3], pieces[row, col + 4], pieces[row, col + 5]))
                    {
                        return true;
                    }
                }
        ...
        private bool CheckColumns(Proxy[,] pieces)
        {
            ...
            for (int col = 0; col < cols; col++)
            {
                ...
                {
                    if (CheckConsecutive(pieces[row, col], pieces[row + 1, col], pieces[row + 2, col], pieces[row + 3, col], pieces[row + 4, col], pieces[row + 5, col]))
                    {
                        return true;
                    }
                }
        ...
        private bool CheckDiagonals(Proxy[,] pieces)
        {
            ...
            for (int row = 0; row <= rows - 6; row++)
            {
                for (int col = 0; col <= cols - 6; col++)
                {
                    ...
                    if (CheckConsecutive(pieces[row, col], pieces[row + 1, col + 1], pieces[row + 2, col + 2], pieces[row + 3, col + 3], pieces[row + 4, col + 4], pieces[row + 5, col + 5]))
                    {
                        return true;
                    }

                    // Check diagonal from upper right to lower left
                    if (CheckConsecutive(pieces[row, col + 5], pieces[row + 1, col + 4], pieces[row + 2, col + 3], pieces[row + 3, col + 2], pieces[row + 4, col + 1], pieces[row + 5, col]))
                    {
                        return true;
                    }
                }
